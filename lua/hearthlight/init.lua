...
M.config = { italics = true, variant = "parchment" }
...
M.load = function()
  ...
  vim.g.colors_name = "hearthlight"

  -- ── Palette ──────────────────────────────────────────────────────────
  local p = require("hearthlight.palettes." .. M.config.variant)
  ...
  -- Identifiers — plain fg so names don't clash with keywords/types
  hi("Identifier", p.fg1)
  hi("Function",   p.green)
  ...
  -- Types — orange for type keywords/builtins, fg1 for user-defined names
  -- The distinction is handled at the treesitter level below.
  hi("Type",         p.orange, p.none, italic)
  hi("StorageClass", p.red)
  hi("Structure",    p.red)   -- 'struct', 'class', 'enum' keyword itself
  hi("Typedef",      p.orange)
  ...
  -- ── Treesitter ────────────────────────────────────────────────────────
  -- Variables — always plain fg so user symbols are neutral
  hi("@variable",         p.fg1)
  ...
  -- Types
  -- @type           = user-defined type names (e.g. `UserService` in a definition) → fg1
  -- @type.builtin   = language built-in types (string, int, bool, etc.)             → orange
  -- @type.definition = the name being defined on the LHS of type MyFoo struct {}    → fg1
  hi("@type",            p.fg1)
  hi("@type.builtin",    p.orange, p.none, italic)
  hi("@type.definition", p.fg1)
  ...
  hi("@constructor", p.yellow)  -- e.g. `UserService{}` composite literal → yellow so it reads as a value
  hi("@operator",    p.pink)

  -- Keywords — all red family; italic on flow-control for rhythm
  hi("@keyword",                   p.red, p.none, italic)
  ...
  hi("@keyword.type",              p.red, p.none, italic)   -- 'type' keyword itself → red
  ...
  -- Semantic tokens
  -- @lsp.type.type covers user-defined type references → fg1 (no colour clash)
  hi("@lsp.type.class",          p.fg1)
  hi("@lsp.type.decorator",      p.cyan)
  hi("@lsp.type.enum",           p.fg1)
  ...
  hi("@lsp.type.struct",         p.fg1)
  hi("@lsp.type.type",           p.fg1)
  ...
  hi("FzfLuaTitle",         p.yellow, p.bg2, "bold")
  ...
  hi("MiniPickHeader",       p.yellow, p.bg2, "bold")
  ...
  hi("MiniJump",            p.bg0, p.orange, "bold")
  ...
  hi("CmpItemKindClass",        p.fg1)
  hi("CmpItemKindInterface",    p.cyan)
  hi("CmpItemKindStruct",       p.fg1)
  hi("CmpItemKindEvent",        p.pink)
  hi("CmpItemKindEnum",         p.fg1)
  ...
  hi("BlinkCmpKindClass",        p.fg1)
  hi("BlinkCmpKindInterface",    p.cyan)
  hi("BlinkCmpKindStruct",       p.fg1)
  hi("BlinkCmpKindEnum",         p.fg1)
